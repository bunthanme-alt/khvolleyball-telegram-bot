...
players_data = {
    ...
    "Thona": 2, "Phatdon": 1, "Lyhour": 2, "Thinhhhh (Wick)": 1, "Salit": 2, "Ngonn": 2,
    "Khai Titi": 1, "មិនា": 1
}
...
async def manual_command(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"🏐 --- លទ្ធផលរៀបចំចាប់គូក្រុមដោយដៃ --- 🏐\n\n🔹 ក្រុម A: {', '.join(team_a)}\n🔸 ក្រុម B: {', '.join(team_b)}")
...
def main() -> None:
    # ⚠️ ដាក់ Bot Token ផ្លូវការរបស់បងនៅទីនេះ
    token = "8066577030:AAEtFhPLBEBql1x1aHFp77UYH6XC1c-AwH0"
    
    # បើកដំណើរការ Web Server (Flask Thread) មុនគេបង្អស់
